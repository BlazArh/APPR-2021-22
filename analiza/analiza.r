...
graf4 <- ggplot(data = tabela3 %>% filter (tip == "populacija"), aes(x=Leto, y=vrednost), show.legend = F) +
  geom_line() + geom_point()+ theme(legend.position = "none") +
  ylab("Število prebivalcev")
graf4

graf5 <- ggplot(data = tabela3 %>% filter (tip == "izobrazba"), aes(x=Leto, y=vrednost), show.legend = F) +
  geom_line() + geom_point()+ theme(legend.position = "none") +
  ylab("Indeks izobrazbe")
graf5

#Švica, trije grafi, skozi leta - dohodek, populacija, izobrazba


tabela4 <- skupna %>% filter(Drzava == "Switzerland")

graf6 <- ggplot(data = tabela4 %>% filter (tip == "dohodek"), aes(x=Leto, y=vrednost), show.legend = F) +
  geom_line() + geom_point()+ theme(legend.position = "none") +
  ylab("Dohodek")
graf6

graf7 <- ggplot(data = tabela4 %>% filter (tip == "populacija"), aes(x=Leto, y=vrednost), show.legend = F) +
  geom_line() + geom_point()+ theme(legend.position = "none") +
  ylab("Število prebivalcev")
graf7

graf8 <- ggplot(data = tabela4 %>% filter (tip == "izobrazba"), aes(x=Leto, y=vrednost), show.legend = F) +
  geom_line() + geom_point()+ theme(legend.position = "none") +
  ylab("Indeks izobrazbe")
graf8
...
skupek2 <- plot_grid(gr_PK_2013, gr_PK_2015, gr_PK_2017, gr_PK_2019)
print(skupek2)
